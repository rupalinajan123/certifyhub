...
{  
    if($package_details['package_type'] == 'private'){

        $temp_array  = array();
        $temp_array['package_details']         = $package_details; 
        $products_view[$package_details['package_id']] = $temp_array;
      }
} 
?>                        
<!--vertical Tabs-->
  ...
  { ?>
    ...
    <?php
        ...
        <?php  } ?> 
          ...
          <?php if($products_res['package_details']['package_type'] == "private") {?>
              <a href="<?php echo $this->config->item('PORTAL_LINK').'/product/offer-order-summary/'.$products_res['package_details']['product_id'].'/'.$private_offer_id ?>" class="btn btn-primary buy-product" data-product_id="<?php echo $products_res['package_details']['product_id']?>" data-package_id="<?php echo $products_res['package_details']['package_id']?>">Buy Now</a>
            <?php }?>
            <!-- <?php// echo $this->config->item('PORTAL_LINK').'/product/offer-order-summary/'.$products_res['package_details']['product_id'].'/'.$products_res['package_details']['package_id']?>"  -->
            ...
            if(isset($products_res['package_details']['package_features_list'])){
              ...
              if($package_features_list_json != "")
              {
                ...
                {               
                  ...
                  <ul>
                  <?php 
                    foreach ($package_features_list_json as $i_key => $features )  
                    {
                      if($fi > 5)
                      {  $fi_data = true;
                        ?>
                ...
                <ul>  
                  ...
                  if($package_transaction_list_json != "")
                  {
                   // $package_transaction_list_arr = json_decode($package_transaction_list_json, true);
                     $package_transaction_list_arr = $package_transaction_list_json;
              ...
              <h6>(<?php echo ucfirst($products_res['package_details']['license_frequency']); ?>)</h6>
                ...
                <ul >  
                  ...
                  if($package_license_list_json != "")
                  {
                    ...
                    $package_license_list_arr = $package_license_list_json;
